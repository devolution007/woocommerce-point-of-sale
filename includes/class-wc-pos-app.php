...
class WC_POS_App {
		...
		if ( wc_pos_is_register_page() ) {
			...
			$wp_styles->queue = array_filter(
				...
				function ( $handle ) use ( $wp_styles ) {
					...
					return $style->deps && in_array( 'wc-pos-app', $style->deps, true );
				}
			);
			...
			if ( 'development' === wc_pos_get_env() ) {
				wp_enqueue_script( 'wc-pos-app', 'http://localhost:4560/.quasar/dev-spa/client-entry.js', [], WC_POS_VERSION, [] );
			} else {
				wp_enqueue_style( 'wc-pos-app', WC_POS()->plugin_url() . '/packages/app/dist/assets/index.css', [], WC_POS_VERSION );
				...
				$version = null;
				wp_enqueue_script( 'wc-pos-app', WC_POS()->plugin_url() . '/packages/app/dist/index.js', [], $version, [] );
			}

			// TODO: use one global variable for all the data: window.wcPosData
			wp_localize_script( 'wc-pos-app', 'Nonces', $this->get_nonces() );
			wp_localize_script(
				'wc-pos-app',
				'UserData',
				// HACK: stops wp_localize_script from casting scalar values to strings.
				[ 'user' => $this->get_current_user() ]
			);
			wp_localize_script(
				'wc-pos-app',
				'SessionData',
				[ 'session' => $this->get_session_data( $register->get_current_session(), true, true ) ]
			);
			wp_localize_script( 'wc-pos-app', 'AppConfig', $this->get_app_data() );
	...
	public function get_nonces() {
		...
		return apply_filters(
			...
			[
				...
				'insert_cash_flow'   => wp_create_nonce( 'insert-cash-flow' ),
	...
	public function get_session_data( $id, $include_totals = false, $skip_totals_query = false ) {
		global $wp;
		...
		if ( $session ) {
			...
			$user_id      = 'yes' === get_option( 'wc_pos_display_total_sales_for_logged_in_user', 'no' ) ? $current_user->ID : 0;

			$session_data = [
				'id'             => $session->get_id(),
				'date_opened'    => $session->get_date_opened() ? gmdate( WC_POS_ISO_DATETIME_FORMAT, $session->get_date_opened()->getTimestamp() ) : null,
				'date_closed'    => $session->get_date_closed() ? gmdate( WC_POS_ISO_DATETIME_FORMAT, $session->get_date_opened()->getTimestamp() ) : null,
				'open_first'     => $session->get_open_first(),
				'open_last'      => $session->get_open_last(),
				'locked'         => $session->get_locked(),
				'cash_flow'      => $session->get_cash_flow(),
				'closing_note'   => $session->get_closing_note(),
				'counted_totals' => (object) $session->get_counted_totals(),
			];

			if ( $include_totals ) {
				$session_data['totals'] = wc_pos_get_session_totals( $session->get_id(), $user_id, $skip_totals_query );
			}
		}
	 ...
	 * @param int|null $grid_id     Grid ID.
	 *
	 * @return array
	 */
	public function get_app_data( $register_id = null, $outlet_id = null, $receipt_id = null, $grid_id = null ) {
		...
		$default_register_id = (int) get_option( 'wc_pos_default_register' );
		$default_outlet_id   = (int) get_option( 'wc_pos_default_outlet' );
		$default_receipt_id  = (int) get_option( 'wc_pos_default_receipt' );
		$default_grid_id     = (int) get_option( 'wc_pos_default_grid' );

		$register_data = $this->get_register_data( $register_id ?? $wp->query_vars['register'] ?? $default_register_id );
		$outlet_data   = $this->get_outlet_data( $outlet_id ?? $wp->query_vars['outlet'] ?? $default_outlet_id );
		$receipt_data  = $this->get_receipt_data( $receipt_id ?? $register_data['receipt'] ?? $default_receipt_id );
		$grid_data     = $this->get_grid_data( $grid_id ?? $register_data['grid'] ?? $default_grid_id );
		...
		foreach ( (array) get_option( 'wc_pos_cash_denominations', [] ) as $item ) {
			$type                           = 'coin' === $item['type'] ? 'coin' : 'note';
			$cash_denomoinations[ $type ][] = [
		...
		return apply_filters(
			...
			[
				...
				'pos'  => [
					...
					'beep_beep_sound'                      => WC_POS()->plugin_url() . '/assets/dist/sounds/beep-beep.mp3',
					...
					'cash_denominations'                   => $cash_denomoinations,
					...
					'display_total_sales_roles'            => (array) get_option( 'wc_pos_display_total_sales_roles', [ 'shop_manager', 'outlet_manager', 'register_clerk' ] ),
					'display_total_sales_for_logged_in_user' => 'yes' === get_option( 'wc_pos_display_total_sales_for_logged_in_user', 'no' ),
					...
					'flexible_order_status_changes'        => 'yes' === get_option( 'wc_pos_flexible_order_status_changes', 'no' ),
					...
					'logo_url'                             => $logo_url,
					'public_url'                           => WC_POS()->plugin_url() . '/packages/app/public/', // @todo fix me
					...
					'signature_required_on'                => get_option( 'wc_pos_signature_required_on', [ 'pay' ] ), // TODO: rename to wc_pos_capture_signature_modes
					'tax_based_on'                         => get_option( 'wc_pos_calculate_tax_based_on', 'outlet' ),
					'tax_display_order'                    => get_option( 'wc_pos_tax_display_order', 'excl' ),
					'tax_display_receipt'                  => get_option( 'wc_pos_tax_display_receipt', 'excl' ),
					...
					'variations_display'                   => get_option( 'wc_pos_variations_display', 'tiles' ),
